    ...
    methods(Static)
        ...
        function mergeDatabases(path, name, database)
            ...
            for f= 1:length(files)
                ...
                Utilities.deleteDuplicateRowsForomEEGMuseAccelerometer(newdb);
        ...
        function real_time = correctRealTime(real_time, arrive_time, sampling_rate)
            ...
            index = find(min_subsequent_diff > 1000, 1, 'first');
            if isempty(index) 
                finished = true;
            else
                finished = false;
                real_time(1:index-1) = real_time(1:index-1);
                real_time(index:end) = Utilities.createRealTime(arrive_time(index:end), sampling_rate);
            end
            if ~finished
                real_time(index:end) = Utilities.correctRealTime(real_time(index:end), arrive_time(index:end), sampling_rate);
            end
        end
    end
